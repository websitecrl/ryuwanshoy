...
useEffect(() => {
  ...
  async function fetchNextChapter() {
    ...
    if (json.data?.chapters?.length) {
      const max = Math.max(...json.data.chapters.map((c: { chapter_number: number }) => c.chapter_number))
      setChapterNumber(String(max + 1))
    } else {
      setChapterNumber('2')
    }
  }
  ...
  function addFiles(files: File[]) {
    ...
    setPages(prev => [...prev, ...imgs.map(file => ({
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        try {
          ...
          const pgRes  = await fetch('/api/pages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chapter_id: chapterId, imageBase64, page_number: i + 1 }) })
                      ...
                      <div style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 6, overflow: 'hidden', border: `2px solid ${page.error ? '#FCA5A5' : page.uploaded ? '#86EFAC' : 'var(--ryu-border)'}` }}>
                        ...
                        {!page.uploading && !page.uploaded && (
                          <button type="button" onClick={() => removePage(page.id)}
                            style={{ position: 'absolute', top: 3, right: 3, width: 18, height: 18, borderRadius: 99, background: 'rgba(28,25,23,0.78)', color: '#FFFBF5', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: 0 }}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 10, background: 'linear-gradient(to top, var(--background) 60%, transparent)', padding: '20px 32px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
       <button
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
              disabled={submitting || !hasSeriesSelected || !hasChapterNumber}
              ...
              style={{
                ...
                opacity: (!hasSeriesSelected || !hasChapterNumber) ? 0.5 : 1,
              }}
            >
              Save as draft
            </button>
          <button disabled={submitting || !hasSeriesSelected || !hasChapterNumber || !hasPages} onClick={() => handleSubmit('publish')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px', borderRadius: 8, border: '1px solid var(--ryu-primary-deep)', background: 'var(--ryu-primary)', color: '#fff', fontSize: 13.5, fontWeight: 600, cursor: 'pointer', boxShadow: '0 1px 0 rgba(0,0,0,0.06)', opacity: (!hasSeriesSelected || !hasChapterNumber || !hasPages) ? 0.5 : 1 }}>
